...
(function () {
    ...
    function execute() {
        ...
        Shp.File.add = function (fileName, fileContent, docLibrary, webUrl) {

            var webUrl = webUrl || _spPageContextInfo.webAbsoluteUrl;
            var url = webUrl + "/_api/web/lists/getByTitle('" + docLibrary + "')/RootFolder/files/add(overwrite=true,url='" + fileName + "')";
        ...
        Shp.File.update = function(fileUrl, properties) {
        	var endpointUrl =  _spPageContextInfo.webAbsoluteUrl + "/_api/web/getFileByServerRelativeUrl('" + fileUrl + "')/ListItemAllFields";
  		    return Shp.File.executeJson(endpointUrl).then(function(data){
	  		    var updateHeaders = {
			            'X-HTTP-Method' : 'MERGE',
			     ...
			     itemPayload['__metadata'] = {'type': data.d['__metadata']['type']};
	     		 return  Shp.File.executeJson(endpointUrl,'POST',updateHeaders,itemPayload);
  		    });  		    
        }
        
        Shp.File.executeJson = function(url,method,headers,payload)  {
        
            method = method || 'GET';
		    headers = headers || {};
		    headers["Accept"] = "application/json;odata=verbose";
		    if(method == "POST") {
		        headers["X-RequestDigest"] = jq("#__REQUESTDIGEST").val();
		    }      
		    var ajaxOptions = 
		    {       
		       url: url,   
        ...
        Shp.File.registerClass('Shp.File');
