(function () {
    ...
    function execute() {
        ...
        Shp.Dialog.PromptDialog._onCancel = Function.emptyFunction;
        Shp.Dialog.PromptDialog._onClose = Function.emptyFunction;
        
        
        Shp.Dialog.PromptDialog.cancel = function() {
        	Shp.Dialog.PromptDialog._onCancel();
        };

        Shp.Dialog.PromptDialog.show = function (title, err, closeMethod, cancelMethod) {
            ...
            { name: 'closeMethod', type: Function, optional: false, mayBeNull: false },
            { name: 'cancelMethod', type: Function, optional: true, mayBeNull: false }], true);
            if (e) throw e;

            Shp.Dialog.PromptDialog._onClose = closeMethod;
            Shp.Dialog.PromptDialog._onCancel = cancelMethod || function() {
            ...
            var $dialog = jq('<div id="promptDialog"  class="modal fade" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-hidden="true" style="padding-top:15%; overflow-y:visible;">' +
                ...
                '<button type="button" class="btn btn-primary" onclick="javascript:Shp.Dialog.PromptDialog.cancel()">Cancel</button>' +
                '</div>' +
                '</div>' +
                '</div' +
                '</div');
